...
> 说明：本清单记录需要人工判定的业务场景及跨页面验收步骤；新增功能可在自动化覆盖基础上复核完整操作体验。凡标注「疑似bug」者，请按用例内「预期」判定当前行为是否可接受，并把结论回填给开发。
...
## 2026-09-22 专项：导入长请求与结果核对

使用合成 Excel 和隔离数据库，核对亮/暗主题、舒适/紧凑密度及键盘操作：

1. 准备 129 单订阅和 2 笔纯运费，模拟确认接口超过 120 秒才返回。页面应持续显示“正在保存整批订单和来源交易”，确认、预览、文件替换和批次设置均禁用。成功后展示本批订单数及运费后续入口，不能在两分钟时误报失败。
2. 模拟确认请求连接中断。页面应显示“尚未收到导入结果”，保留文件和预览，禁用原确认按钮并提示先核对订单及来源交易；不能自动重试。核对服务器结果后重新预览：已保存记录判为重复，未保存记录继续走原人工确认流程，旧连接错误在新预览成功后清除。
3. 核对实际保存的订单、来源原件、关联、金额和审计数与批次摘要相符，检查明细及履约目标完整、无重复订单。模拟数据库提交成功后不可读，确认接口仍应直接返回保存结果；模拟提交前失败，则整批回滚且保留草稿供重试。
4. 真实批次报错时，在获得相应数据库访问授权后用只读方式核对落库结果，不能仅凭前端错误重放导入；真实订单明细、电话、地址和连接信息不得写入测试、截图或仓库文档。

## 2026-09-22 专项：导入预览分类与提示布局

使用合成数据核对亮/暗主题、舒适/紧凑密度及窄屏：

1. 预览分类始终只有「全部、待确认、来源更新、重复、可导入、跳过」六个入口；数量为 0 也可通过鼠标或键盘进入空结果，原分类与批次计数保留。
2. 同时有期号和识别必核项时，顶部只显示一条「导入前请完成核对」。两种入口分别筛选对应记录；完成其中一种后，另一种仍显示且整批确认仍禁用。全部完成后提示消失，当前辅助筛选可通过「返回全部」退出。
3. 来源交易、刷新草稿、批量补订期与运费关联位于上方工具区。运费筛选不新增结果分类；识别筛选中的「处理下一条」能进入原核对流程。只读账号可筛选，不能刷新、补订期或执行核对；刷新期间筛选与核对入口禁用。
4. 核对待确认、来源更新、仅保存交易的说明仍可见；筛选和界面调整不改变整批确认范围及原有校验。列表仅在表格内部横向滚动，标题、工具、分类和提示不得横向溢出。

## 2026-09-11 专项：导入预览、商品关联与批量补订期

新增核对与运费关联验收（使用合成 Excel 和隔离数据库）：

1. 原生中通商品无转投警告；同单邮局订阅和转中通运费生成逐明细核对。列表或详情均能确认保留邮局或改中通，订期随选择重算；未核对直接提交应阻断。
2. 未知状态必须选择实际状态，选待付款后跳过；实付低于套餐固定额时须修正为非负且总额相符的分摊。缺日期、未知商品能更正本单，原始快照不变。
3. 在历史模式补人工订期后修改投递，原日期保留且要求重新确认；往期期号、商学院期次在列表和详情共用填写状态。重新预览前提示核对将丢失。
4. 新运费推荐已有与本批订阅，缺资料、多候选只显示疑似；可跨目标分摊，金额不等不得保存。无候选可留存，导入结果能逐笔继续关联。仅关联不改变订阅份数、财务或投递。
5. 保存关联草稿后修改目标资料或本批订阅，正式提交应阻断；刷新并重新选择后恢复。模拟关联或审计写入失败，应无半笔订单、来源或关联且可重试。未知状态识别规则升级后，旧原件不变化仍应判重复。
6. 核对亮/暗主题、舒适/紧凑、窄屏滚动、键盘操作、加载/空/错误/只读状态；普通用户不得通过直接调用核对及关联接口写入，其他人和过期会话不可操作。

**翻期临界期号核对（隔离环境、合成数据）**：准备付款时间为周五 23:00 的最新一期订单，含两条单期明细，并把该单排在第 2 页。预览后整批确认应禁用；通过「期号待核对」找到该单，第一条确认建议期号，第二条在详情中改选另一刊期并确认。列表与详情期号及已核对状态应同步，完成后整批确认启用。修改已确认的期号应撤销该条核对；切换分类/分页保留结果，重新预览须提示清除并重新核对。确认后检查各明细期号、实付、来源快照和订单审计；未核对直接调用提交接口、预览后修改刊期日期或传无效期号，都应整批失败且保留可重试会话。核对亮/暗主题、舒适/紧凑、只读、无可选刊期及键盘操作。

以下步骤仅在隔离测试环境使用合成订单。自动化和 Storybook 已覆盖核心行为；部署后的完整页面操作按以下步骤复核，本清单不代表生产验收已完成。

分类按 [导入结果边界](order-import-decision-rules.md) 核对：准备退款且商品已匹配、退款且商品未知、待付款、全为忽略商品、缺日期及混合商品交易，并区分无已有记录、只有业务订单、已有来源原件。确认五类页面数量之和等于全部，且没有独立“留存”标签；纯运费及旧订单补原件计入“可导入”，行内说明仅保存交易。退款加未知商品或缺日期进入待确认，补齐后可重新识别；若先确认其他记录，未解决退款不得提前保存成来源或变成重复。纯运费待付款也可保存来源，已有来源优先判断重复或来源更新，只有业务订单加取消仍跳过。切换筛选后确认范围仍是整批可导入记录/来源更新；漏勾来源更新须阻断整批且不写入。确认后按摘要分别核对新建订单数和仅保存交易数，更新保留新旧原件，原订单资料和状态不被重导修改。分别核对预览重复数和确认时新增发现的重复数。

1. **关联同款促销**：准备 17 单同名促销订阅，实付 ¥199，现有促销商品参考价 ¥240、保留一个旧别名。上传后从待确认汇总点击「关联已有商品」，用名称或编码选择并核对期限、刊物、投递方式，保存别名并重新识别。预期同名 17 单变为可导入，商品总数不增加、旧别名保留、成交价仍为 ¥199；其他待确认问题继续单独列出。检查未选择、无可选商品、读取失败重试与名称冲突提示；只读角色无关联或新增入口。
2. **导入前分批补录**：使用历史归档模式，让合成订阅的起止日期留空。进入「批量补订期」，按刊物/投递筛选并跨页多选，填入 2026-03 起始月、全年期限。核对窗口应显示 2026-03-01 至 2027-02-28；双刊可分别使用不同日期，自定义期限必须明确填写。应用后返回导入预览，正式确认导入前不建单。
3. **重新识别与草稿保护**：先应用一批订期，再关联商品、修改导入设置或更换文件。预期提示重新预览会清除已补日期；选择保留后当前预览和日期不丢失，选择继续后按新设置重新生成预览。
4. **导入后找回与一致性**：只补部分明细后确认导入，通过「继续补本次订期」或订单列表「待补订期」找回余项。仅缺一端的明细应保留已有日期，退款/取消/待付款行禁选。保存后已补齐行退出待补队列；核对原金额、收件人、履约版本和已有发货明细未变，日期差异、原因和操作人可在事件流追溯。
5. **冲突与页面状态**：在一个窗口预览补订期，另一个窗口修改该订单后再确认，预期整批拒绝；刷新并重新填写后才能保存。检查亮/暗主题、舒适/紧凑密度、键盘操作、空队列和失败重试；普通订单编辑页只补日期时，导入成交价保持不变。
6. **预览分类筛选与订期展示**：准备超过 50 单的混合结果，让重复和待确认订单位于原列表第二页。先补期号/期次，再切换「待确认」「重复」「跳过」「可导入」，检查筛选覆盖整份预览、回到第一页、已补内容保留，分类计数始终为整批数量。重新识别后保持当前分类并刷新数量，结果为零时明确显示空态；更换文件或设置后回到「全部」。在「重复」分类下确认导入，仍应按按钮所示数量处理整批可导入订单。列表和识别详情的订期须显示起止日期，缺失日期显示「未填写」；键盘可切换分类，选中标签在亮暗主题中均清晰可辨。
...
---
**[P0] 马飞—库房留存从错误无需运单改为库存入库**
- 前置：某期确认版发货计划存在“马飞 / 库房留存 / 71份”，上传文件中该行没有运单号，并被解析为无需运单；可另为该明细预置一条小于计划数量的错误“无需发货”核销。
- 步骤：
  1. 上传文件进入运单核对工作台，检查马飞行的匹配状态和提示。
  2. 分别核对表格操作区的「转库入库」和「核对」弹窗内的「改为转库留存/库存入库」入口。
  3. 点击转换并确认，再确认导入批次。
  4. 返回实际发货汇总和已忽略导入行，核对数量、分类和审计原因。
- 预期：预览阶段马飞行显示为待处理，不计入普通已匹配或实际寄出；转换后原行保留为已忽略凭证，错误无需发货记录改为 `warehouse_stock_in`，系统补足到完整计划 71 份；实际发出为 0、转库留存为 71、未解释待补为 0，确认导入不再被马飞行阻断。若该明细已有真实运单、实发或合寄记录，转换必须返回冲突提示且不修改数据。
- 为何人工：需要同时核对两个页面入口、确认弹窗、导入行筛选、汇总口径和真实浏览器交互；后端原子转换与冲突保护另有自动化测试。

---
**[P1] 发货状态卡、运单确认栏及禁用主按钮文字对比度**
- 前置：分别准备“实际发货仍有待处理”“实际发货接口失败”“运单导入存在已匹配行”和“计划缺口未勾选明细”的页面状态，并同时核对亮色、暗色主题。
- 步骤：查看单期快递管理中的「继续处理」「重新加载」按钮、运单工作台底部「导入已核销的 N 份」主按钮，以及未勾选明细时禁用的「标记每月两次合寄」；另打开一个确认条件尚未满足、主按钮禁用的弹窗。
- 预期：状态卡说明文字继续使用次要文字色，但按钮内部文字不继承说明文字样式；启用的蓝色主按钮文字保持白色。普通区域和弹窗内的禁用主按钮均使用 Ant Design 主题感知的禁用背景与文字，不保留品牌蓝底或蓝色渐变；弹窗的容器、表单与 footer 同步使用当前主题背景、边框和文字变量，亮暗主题下文字都清晰可辨。状态卡与确认栏的标题、说明仍按原布局分行显示。
- 为何人工：需要核对 CSS 后代选择器及全局主按钮规则不会误伤 Ant Design 按钮内部元素和禁用态，并检查两种主题下的实际视觉对比度；Storybook 交互测试覆盖启用文字色、普通禁用主按钮背景和弹窗禁用主按钮背景。

---
**[P0] 0份占位统一过滤与存量清理**
- 前置：准备系统单表模板和原始多工作表文件，各放入正常正数行，并分别在“每周（对公）”“每周（读者）”“停发-双周（读者）”等页签加入0份行；其中至少一条姓名、电话、地址全空。数据库升级前确认存在预期的6条无履约关联0份占位。
- 步骤：分别走往期双文件预览和单期“上传/替换计划”预览；再尝试手工新增0份、把正数明细编辑为0份；升级迁移后按受影响期次查看计划与实际发货。
- 预期：两种预览均提示“已忽略 N 条0份发货明细”，导入条数、份数、抽样和提交结果不含任何0份行；手工新增/编辑返回校验错误；迁移只删除无履约依赖的0份占位并写审计日志，若0份行意外关联运单、核销、延期或投诉补发则阻断升级而不静默删除。受影响期次不再在计划或实际发货显示“(未填写) / 0份”。
- 为何人工：需要用真实 Excel 同时覆盖多个原始页签、预览提示和迁移后的页面消失效果；解析器、API校验、数据库正数约束和迁移前置检查另有自动化测试。

---
**[P0] 计划保留停发行时自动生成无需发货归因**
- 前置：某期确认报数和计划总数均为1,414份，其中“测试停发客户”1份当前为“正常”，其余明细合计1,413份；该明细没有运单、合寄或转库记录。
- 步骤：在“发货计划”勾选“测试停发客户”并点击“设为停发”；分别查看顶部计划对账、计划列表汇总、实际发货核销卡、该客户实际发货卡片和“查看变更与归因”；再把该客户恢复为“正常”。
- 预期：设为停发和创建 `source=plan_status` 的1份无需发货记录在同一事务完成；顶部显示“确认报数1,414、当前计划1,413、已归因停发1、未解释差异0”，计划列表显示“计划总数1,414、应发1,413、停发1”，实际发货显示“计划总数1,414、无需发货1、核销待补1,413”，该客户不再显示待发。归因来源显示“计划停发联动”，转库留存数量不进入“已归因停发”。重复保存不重复建记录；恢复正常后只撤销系统自动归因并回到当前计划1,414。已有实发、待合寄或转库记录时设为停发必须返回冲突且整批回滚。
- 为何人工：需要同时核对两个页签、抽屉状态、汇总文案和恢复操作；原子联动、幂等、冲突与对账公式另有自动化测试。

---
**[P0] 1,321 份确认印数：955 份自动匹配 + 365 份拆单人工关联 + 1 份客户暂停核销**（运单未匹配不得丢失）
- 前置：2026-01-26 期确认印数和确认时计划均为 1,321 份；确认后某客户要求本期暂停发货 1 份，当前计划变为 1,320 份。当前计划含“肖波 / 成都杂志铺 / 365份”。运单源文件共 1,320 份，其中 955 份可自动匹配，肖波的 365 份拆为 65、100、100、100 四个运单且初始未匹配，暂停发货的 1 份不在源文件中。
- 步骤：
  1. 上传文件进入运单核对工作台，检查顶部两条状态和待处理列表。
  2. 先确认 955 份已匹配运单，再次进入同一批次。
  3. 选择四条肖波运单，批量关联到“成都杂志铺 / 365份”计划明细。
  4. 在“计划缺口待归因”中登记 1 份“无需发货”，选择实际暂停的收件明细，确认原因默认选中“客户要求暂停本期发货”。
  5. 再准备一条历史无 `shipping_detail_id` 的无需发货记录，确认页面显示“待补充归属”，随后在工作台补选对应明细。
- 预期：首次预览显示源文件 1,320 份、已关联 955 份、待人工关联 365 份、计划缺口待归因 1 份；确认 955 份后四条未匹配运单仍完整可见且可编辑。批量关联后生成四个独立包裹，合计 365 份。登记并归属暂停寄送后顶部显示“计划已对平”，公式为“1,321 = 当前计划 1,320 + 已归因停发 1”；实际状态显示“核销已完成 · 部分发货”，实际发出 1,320、无需发货 1、核销待补 0。“查看变更与归因”能看到收件人、渠道、原因和份数。历史无归属记录在补齐前不对平计划，补齐后才变为已归因。

**[P0] 转库留存后仍正确显示 24 份逐明细计划缺口**
- 前置：某期计划应发 1,421 份，运单匹配 1,325 份，已登记“马飞—库房留存”72份；核销摘要返回 19 条尚未归因明细，`remaining_quantity` 分别为 4、3 和 17 个 1，共 24 份。
- 步骤：进入运单核对工作台，检查默认查看对象、计划缺口计数、明细行和顶部“未解释待补”。
- 预期：导入行无待核对项时默认选中「计划缺口 24份」；查看对象卡片和提示均显示 24 份，19 条明细完整可选，不得因 72 份库存入库已反映在逐明细差额后又被重复扣减成 0；切回「导入明细」后，一级状态满足“全部导入 = 已匹配 + 待核对 + 转库留存 + 已忽略”。月底建议显示金色「待确认」和蓝色「月底合寄」，登记后显示蓝色「月底合寄」与独立的「已登记 · N份」。
- 为何人工：需要核对接口汇总、默认筛选和表格标签三处展示一致，并确认颜色在亮/暗主题下没有把业务类型与处理状态混淆。

**[P0] 每月两次合寄与月底合寄分流**
- 前置：分别准备一个 4 期自然月和一个 5 期自然月，同一客户每期各 1 份；另准备一个整月月底合寄客户。
- 步骤：4 期月份将第 1、2 期登记为每月两次合寄并在第 2 期完成一张运单，第 3、4 期在第 4 期完成第二张；5 期月份按第 1+2 期和第 3+4+5 期完成两张；月底合寄客户将当月全部期次在最后一期完成一张。另准备一条没有目标期号和目标日期的历史待办，分别查看快递管理总览、非目标期和目标期。
- 预期：工作台分别统计“待每月两次合寄”和“待月底合寄”；快递管理总览显示全系统累计未完成条数与份数，并把无目标记录列为“历史未分批”。非目标期不显示合寄按钮；第 2 期和最后一期只显示目标批次恰好为本期的「本期待完成合寄 X条 / Y份」，历史未分批不得混入。4 期月形成 2+2、5 期月形成 2+3、月底合寄形成 4 或 5 期一批；一张实际运单按份数分摊到各来源刊期，完成前不计实际发出，完成后各期核销均归零。
- 为何人工：重点验证确认后的交互可恢复性、批量关联操作、顶部单行布局以及源文件原始行是否完整可追溯。

---
**[P1] 高铁运单表兼容两种收件人列布局**
- 前置：2026-05-18 期存在“高铁展示”发货明细“赵叶 / 15810698235 / 5份”；准备两份标题含“高铁”的运单工作表：一份 C/E/F/G/H 依次为运单号、电话、地址、姓名、份数；另一份 C/E/F/G/H/I 依次为运单号、电话、地址、展示名称、姓名、份数。
- 步骤：分别上传两种高铁运单文件；若同一文件已有旧草稿，点击「重新上传并解析」。
- 预期：紧凑布局读取 G 列“赵叶”和 H 列 `5`；带展示名称的布局读取 H 列“赵叶”和 I 列 `5`，G 列展示名称只作为原始数据保留。两种布局都进入正常自动匹配，不显示“未识别 / 无效”。
- 为何人工：需要同时核对真实 Excel 列布局、页面展示字段和自动匹配结果，防止份数错占收件人列或展示名称被误作收件人。
...
---
**[P1] 分次开票累计金额上限与状态流转**（数据正确性）
- 前置：生效订单 O，应开金额 240，invoice_required=True，尚未登记正票。
- 步骤：
  1. 登记第一张正票 100，保存。
  2. 确认工作台显示「已开 100 / 待开 140」，状态仍为「待开票」，操作变为「继续开票」。
  3. 继续开票，确认弹窗默认金额为 140；改填 141 保存。
  4. 再填 140 保存，随后尝试通过 API 再登记正票 1 元。
- 预期：第一张保存成功；141 被拒绝并提示超过待开 140；140 保存后累计已开 240、待开 0，状态变为「已开票」且不再显示正票登记入口；额外 1 元请求被后端拒绝。
- 自动化覆盖：`test_normal_invoice_can_be_split_but_cannot_exceed_order_total` 覆盖接口累计上限和状态流转；人工仅确认页面金额、按钮文案及默认值。

---
**[P1] 电子发票附件选填、补传与预览**（财务归档）
- 前置：一张尚未登记发票的待开票订单；准备一份 PDF 或 PNG 电子发票。
- 步骤：
  1. 管理员打开「登记发票」，不选择「电子发票（选填）」直接保存。
  2. 进入「查看记录」，确认该条显示「未上传电子发票」，点击「补传」上传文件。
  3. 分别点击「预览」「下载」，再用「替换」上传另一文件。
  4. 点击「删除附件」，确认开票记录仍在；切换普通操作员账号查看同一记录。
- 预期：未上传附件不阻塞登记；补传后文件名正确，PDF / 图片可在页面预览并能下载；替换后展示新文件；删除附件只清文件、不改变开票金额与状态；普通操作员可预览 / 下载但看不到补传、替换和删除附件入口。单个文件超过 20MB 或扩展名非 PDF / JPG / PNG 时被拒绝。
...
---
**[P1] 新增结算弹窗紧凑布局与识别归属**（财务·结算·视觉回归）
- 前置：管理员账号；至少一个可用合作渠道；准备一份可识别的北京报零 XLSX 和一份发票 PDF。
- 步骤：
  1. 进入「财务管理 → 渠道结算」，点击「新增结算」。
  2. 在桌面宽度检查标题、结算凭证、结算基础信息、周期与金额以及底部保存区。
  3. 先上传 XLSX，再上传发票 PDF，观察识别提示和附件行。
  4. 填写完整退报日期区间，分别缩窄到约 600px 和 420px 检查重排。
- 预期：桌面基础信息按三列紧凑排列；结算周期与退报周期并排等宽且完整容纳日期区间，三个金额同排，备注为正常字号单行输入。识别成功/警告紧贴 XLSX 行并明确写“此结算单识别结果”，不会看成下一份附件的状态；附件类型已由下拉表达，不出现“主结算单/已归类”蓝色标签。多份结算单时仅其他表显示“改用此表识别”。约 600px 降为两列、约 420px 降为单列，字段和按钮不重叠。
...
**补充：订报转投批次的投递单位核对（2026-09）**

- 用合成数据准备至少 55 条记录、两个投递单位和另一个批次。设为有效后核对原省份分配结果不变，窗口自动打开；关闭后从批次详情“投递单位”按钮重新进入。
- 第一页与第二页各勾选一条并批量修改；保存汇总应只包含两条。再测试“本批次全部记录”统一分配并设置一个逐条例外，汇总与实际保存数须跨页一致，其他批次和归档行不受影响。
- 在同一起投月另建一条未关联批次的合成历史记录。投递明细按年度和起投月筛选时应包含它，批次核对窗口不包含它；整批调整后它的单位和来源关系保持原值。另验证仅有历史记录时，批次窗口为空但起投月列表仍有记录。
- 有未保存调整时关闭窗口应提示放弃；保留当前分配不撤销版本激活。重新激活后仍恢复原自动分配并再次弹窗。
- 两个窗口同时打开同一批次，先在一处保存，另一处保存应提示重新核对。确认失败或网络重试不能造成部分记录更新或重复审计。
- 检查亮/暗主题、舒适/紧凑密度、加载/空/错误状态，以及键盘切换选项、翻页和确认操作。演示账号不显示处理入口，直接请求修改接口返回403。
...
| P0 | 订报转投中途失败须保留上一套当前产物并清理半成品 | 邮局 |
| P1 | 自定义完整订期单价须贯穿版本金额与全部 Excel 产物 | 邮局 |
| P1 | 应用改地址精确修改绑定目标；多目标未绑定时阻断 | 邮局 |
| P1 | 已应用改地址工单锁定为只读，编辑/删除均须阻断 | 邮局 |
...
详情可读性：使用合成长地址、多商品、开票换行、空字段、未知平台状态及原表独有备注的预览，检查详情字段名不竖排、无横向溢出、不显示英文状态或整块对象代码；金额保留两位小数、日期正常分隔，电话号码和邮编的前导零保留。展开“开票及其他信息”和“查看原表信息”，核对文件、工作表和行号。对来源更新分别构造新增、备注清空、字段移除、商品数量变化及格式变化，所有变化须能逐项核对；未勾选不得保存整批，重新打开保留勾选，重新预览清除旧勾选。检查亮/暗主题、舒适/紧凑密度和窄屏，键盘可展开补充信息、勾选确认及关闭详情。只读角色及预览/提交进行中不能勾选，缺少前后快照时提示重新预览。
...
---
**[P0] 应用改地址：回写投递记录 + 精确修改订单当前履约目标（多目标不猜测）**
...
- 预期：`apply_address_change`——写回投递记录 recipient_*（`new_address` 经 cpca 拆省市区）、`new_copies` 改份数；D 有 `fulfillment_target_id` 时只修改该 active + post_office + 当前 allocation 目标；未绑定时仅在候选唯一时自动绑定并修改；候选超过 1 个或绑定目标失效→409，且名册和订单均不写入。重复应用→409「已应用，请勿重复」；AC2 未关联→400「未关联到投递记录…请先导入读者名册」。
- 为何人工：需在 UI 用单目标、精确绑定的多目标、未绑定的多目标三种真实订单核对阻断提示和最终收件人。

---
**[P1] 已应用改地址工单保持只读**
- 步骤：1) 应用一张改地址工单。2) 在统一列表和详情抽屉查看操作区。3) 直接调用统一及旧兼容 `PUT`/`DELETE` 接口尝试修改、删除。4) 重新查看工单和已写入的名册/履约目标。
- 预期：列表显示「已锁定」，详情显示「已应用 · 只读」，不再提供编辑/删除入口；四个写请求均返回 409，原工单、应用留痕和目标地址保持不变。如需再次更正，应新建改地址工单。
- 为何人工：需核对 UI 只读状态与后端强制守卫一致，避免只隐藏按钮却能绕过接口篡改。

---
**[P1] 新建邮局工单从投递明细关联读者**
- 步骤：分别新建投诉、改地址、回访，依次用“年度-编号”、带前导零编号、姓名、电话、地址片段搜索同一读者并选择。
- 预期：各搜索方式均返回同一条明确的名册记录；选中后年度、编号、姓名等字段自动带入且新建时不可手改。改地址还应带入原电话、原地址、原份数、原起月日，只填写修改后信息即可；保存后的 `postal_delivery_id` 必须命中所选记录。
- 为何人工：需验证异步下拉、长地址辨识、选择后表单回填，以及三类工单交互一致性。

---
**[P1] 邮局信息变更只补起投日期**
- 步骤：准备一张已应用的合成工单，含一份已变更但缺日期的收件人与一份待确认去向。打开工单详情，点已变更收件人下「补充起投日期」，只选择日期并保存；刷新查看。再以普通用户、已有日期和只读来源入口核对；另在两个窗口对同一去向补不同日期。
- 预期：弹窗自动展示姓名、电话、地址、份数，唯一需要输入的是日期；空日期阻断。保存后仅所选去向显示该日期，另一份待确认提醒继续保留；原始工单和投递订期保持不变，审计记录含操作人、时间、所选去向和补充日期。同日期重试不重复留痕，第二个窗口不能覆盖已填日期。普通用户、已有日期和只读来源无此入口。亮/暗主题及舒适/紧凑密度均可正常操作，键盘能打开弹窗并选择日期。
- 为何人工：需核对补日期与确认剩余收件人的入口不混淆，已有收件信息清楚可辨，保存后局部数据与提醒刷新一致。

---
**[P1] 邮局工单卡片布局与电话号码模糊搜索**
- 步骤：分别准备投诉、信息变更、回访工单，电话包含纯数字、空格和连字符三种形式；进入统一列表，依次输入电话号码中间片段，并切换类型、年度和状态筛选。
- 预期：列表使用圆角工单卡片，依次显示编号、收报人（姓名 / 电话）、工单类型、处理情况、登记日期、状态和操作，不展示工单内容摘要；年度编号数字部分不足 4 位时左侧补零；投诉显示处理次数，信息变更显示投递信息是否更新，回访显示已记录；详情、编辑、删除图标纵向对齐。电话中间片段均可命中对应工单，空格和连字符不影响搜索，筛选计数与结果同步。
- 为何人工：需核对响应式卡片布局、字段层级和电话号码展示，同时确认组合筛选的真实交互。

---
**[P1] 改地址修改日期精确到分钟**
- 步骤：新建改地址工单，选择具体日期和时间后保存；在统一列表和详情抽屉核对，再编辑未应用工单修改时间；另导入一条只有修改日期的历史 Excel。
- 预期：统一工单列表的三类工单均只显示 `YYYY-MM-DD`，不因工单类型混用日期格式或额外换行显示时分；改地址表单和详情显示 `YYYY-MM-DD HH:mm`，接口保留秒值；历史 Excel 日期落为当天 `00:00`，既有日期迁移后不丢失。
- 为何人工：需核对统一列表的日期格式一致，同时确认 DatePicker 与详情仍保留分钟精度。
...
---
**[P0] 往期导入以“每周合计”为主期号，跨期页签不得抬高主期号**
- 前置：印数文件为第 2635 期；原始中通文件 `每周合计!B1` 为第 2635 期，`月底-整月!C1` 同时包含 2635、2636、2637、2638，`停发-双周（读者）!C1` 包含 2551、2552；文件名含“（2635）”。
- 步骤：在「历史印数期数 → 导入往期」上传两份文件并点击「预览导入」，不确认提交。
- 预期：预览显示“第 2635 期”和“发货期号来源：每周合计!B1”；不会识别为 2638；月底和停发页签的明细仍正常计入条数/份数；两份文件其余校验通过时允许确认导入。

---
**[P0] 原始中通周发页签期号冲突时阻断并定位来源**
- 前置：原始中通文件 `每周合计!B1` 为第 2648 期，任一周发页签（例如 `每周（读者）!C1`）改为第 2649 期。
- 步骤：与第 2648 期印数文件一起上传并预览。
- 预期：预览不可确认；错误明确显示“主期号冲突”、`每周合计!B1 为 2648 期` 和冲突页签/单元格/期号，不以最大值或任意首个值继续导入。

---
**[P1] 缺少“每周合计”时仅从一致周发页签回退，文件名不覆盖工作簿**
- 步骤：1) 上传没有 `每周合计`、但所有现有周发页签均为第 2648 期的文件，核对预览来源。2) 再把文件名改为“中通快递发货明细（2649）.xlsx”后重新预览。
- 预期：两次都识别第 2648 期，来源显示首个命中的周发页签与单元格；第二次额外显示文件名 2649 与工作簿 2648 不一致的黄色提示，但文件名不覆盖主期号。若周发页签之间不一致，则按上一用例阻断。

---
**[P1] 补录其他期数展示完整出刊日期**
- 步骤：打开「发行计划 → 印数管理」，展开「补录其他期数」下拉框，核对多个跨年份或跨月份的候选刊期，再任选一期。
- 预期：每项及选中值均按 `第 X 期（出刊日期：YYYY-MM-DD）` 显示；日期取自刊期表 `publish_date`，不使用创建时间、上传时间或操作时间。

---
**[P1] 报数页期次页头随长页面滚动吸顶**
- 步骤：在桌面宽度分别打开一期待确认和一已确认报数，向下滚动到来源凭证、修订历史和底部操作区域；再将窗口缩窄到页头按钮发生换行后重复滚动。
- 预期：期次标题、出版日期、状态和全部操作按钮作为一整条固定在全局导航正下方；滚动过程中始终能看出正在处理哪一期，页头不透明、不被内容覆盖、不发生横向溢出，按钮仍可点击；回到页面顶部后布局和圆角正常。

---
**[P0] 来源文件统一卡片覆盖原始、待确认、确认后凭证和跨期文件**
- 前置：准备一份周报原始来源、一份成都月度印前追加、一份确认后仅归档凭证，以及一份成都跨 4 期凭证。
- 步骤：分别进入各关联刊期的「数据来源与调整」，检查文件卡并点击下载。
- 预期：所有文件使用同一套卡片，均展示规范名、原名、本期份数、用途、状态、文件大小和下载入口；已确认印前来源显示确认计入数，若与 OCR 数不同则同时显示原始识别数；确认后凭证显示凭证记录数；跨期文件显示唯一刊期数与期号列表，同一期多行不重复计期；仅归档明确显示“不改变印数、结算或补发”。

---
**[P0] 已确认来源可直接更正人工核对数字且保留旧版本**
- 前置：草稿期存在一份已人工确认的周度 / 月度来源，其中至少一个「来源数字」或「写入份数」由人工填错，原归档文件本身正确。
- 步骤：1) 在该来源卡片点击「更正核对数字」。2) 确认无需选择文件，期号、项目、核对状态及删除 / 人工补录入口均锁定，只修改错误数字。3) 核对“替换贡献、来源合计”前后值后提交。4) 返回来源栏检查新旧卡片和当前报数，再查看操作日志。5) 另走「重新上传文件」选择同一原文件，核对提示。
- 预期：提交后当前印数按更正值重算；新卡片显示「人工更正版」，旧卡片显示「已替换」且仍可下载；两版 SHA-256 相同，新明细通过 `supersedes_item_id` 指向旧项；日志包含 `correct_source` 和 `confirm_source`。同文件普通重传仍被去重，并提示使用「更正核对数字」，不得静默关闭或要求用户人为修改文件。

---
**[P1] 来源栏宽度、长文件名和渠道分类份数**
- 步骤：在桌面宽度打开含四个渠道来源的报数页，检查规范名、原名和各渠道卡片明细。
- 预期：右侧栏向左拓宽且不挤坏左侧两列表单；长文件名完整换行；北京邮发显示本市、外埠（含损失分摊）及各自份数，北京报零显示东部、西部及份数，广州日报显示订户数、零售数及份数；成都杂志铺只显示本期数据，不重复无意义分类。

---
**[P0] OCR 零明细 / 刊期未解析文件不可丢失或绕过确认**
- 步骤：上传一份无法识别结构化数字的图片后直接关闭抽屉并刷新页面，再尝试确认印数。
- 预期：文件仍固定在上传当期，显示「待关联刊期」或「OCR待核对」及「继续核对」；确认印数被阻止。人工补录刊期并确认后状态变为「已人工确认」，文件仍可下载。

---
**[P0] 跨期月度来源按本期阻断且继续核对可幂等提交**
- 前置：成都月度文件关联连续多期；当前草稿期明细已人工确认且与印数一致，另一期明细仍为 `OCR待核对`；当前文件已是本期有效基础来源。
- 步骤：1) 在当前期确认报数。2) 返回来源卡点击「继续核对」，检查已确认行的用途，完成另一期核对并提交。3) 进入原待核对期确认报数。4) 另上传一份同项目基础来源并按基础来源提交。
- 预期：步骤 1 成功，其他期待处理只显示跨期提示；步骤 2 不要求重新上传、不提示选择追加或定向替换，已确认行保持原基础 / 追加 / 归档用途；步骤 3 在本期核对完成前仍被阻断，完成后成功；步骤 4 仍以 409 阻止第二份基础来源。

---
**[P1] 来源文件实体异常提示**
- 前置：测试环境中令一条来源文档的相对路径无效或移走对应文件。
- 预期：文件卡显示「文件异常」，下载按钮不可用；其他正常文件仍可展示和下载。
...
3. **应用新地址**：`apply_address_change` 回写投递记录 recipient_*（cpca 拆省市区）+ 精确修改订单当前邮局履约目标；未显式绑定时只接受唯一候选，多目标返回 409。检查点：下一版月度明细使用新地址；多目标订单绝不误改其他收报人。
4. **锁定已应用工单**：应用后的改地址工单进入只读状态，编辑/删除统一返回 409；如需更正只能新建工单。检查点：工单内容、应用事件与已经写入的名册/履约地址始终一致。
5. **投诉三态处理**：`add_handling`/`delete_handling` 驱动 open↔in_progress↔resolved（无方向校验、可跳档/回退）（`postal_complaint_service.py:200-281`）。检查点：三态跳转是否符合预期；删处理记录回退正确。
6. **删除投诉保留回访**：建立同编号投诉和回访，确认回访先并入投诉时间线；删除投诉后，投诉及原时间线消失，但回访记录应保留并重新出现在独立回访列表中。数据库中该回访的 `parent_ticket_id` 应为空。
7. **删除守卫逐项列明**：删除合作渠道时统计合同/结算/履约目标/投递记录/邮局投递明细/投诉六类引用，任一非零→409 逐项列明（Part A 客户·合同 P1，`partners.py:98-141`）。检查点：让某渠道同时被投递记录 + 投诉引用，删除应 409 且列明「投递/投诉」条数。
# 邮局投诉 → 中通补发 → 订单同步

1. 在邮局工单打开一张已关联订单的投诉，创建包含多个刊期的中通补发任务。
2. 确认补发卡显示收件快照、期次和份数，工单时间线出现“创建中通补发”。
3. 进入对应刊期 ZTO-MF，确认来源为“投诉补发”，展开后可追溯投诉工单、补发任务、来源订单和邮局投递。
4. 从 ZTO-MF 登记全部补发行发出，返回邮局工单确认任务反向变为“已发出”且显示运单号。
5. 在订单详情确认履约目标显示投诉/补发提示，“关联邮局”和“关联快递”均展示同一任务；主履约进度不因补发增加。
6. 新建另一待发任务并取消，确认对应 ZTO-MF 行被移除但工单仍保留“已取消”审计卡。

### 刊期表出刊进度与历史年份（2026-09）

- 使用合成刊期，覆盖出版日前一天、当天、未来期、休刊、计划/实际版数不一致，以及 2024 年仅有刊期表的情况；分别查看亮/暗主题与舒适/紧凑密度。
- 预期：按北京时间日期显示已出刊/未出刊，下一期蓝框，休刊不计入进度；历史无印数记录显示“已出刊（按刊期）”“计划版数”“未录入印数”，年度无可比版数显示“—”。已有实际版数的未来期仍显示未出刊。
- 筛选未出刊及单月后，原月内列位置保持不变，年度/月度进度仍按全年/整月计算；检查空、加载、错误状态。用键盘选择月份、重置筛选，并聚焦橙色版数标签查看计划与实际值。
- 页面停留至北京时间零点，或跨日后恢复前台，出刊状态与下一期应自动更新。无真实数据时仅进行合成数据验证，不访问生产库。

### 来源交易留存验收

用合成 Excel 上传已发货及已退款纯运费各一笔：预览均为“可导入”，行内注明纯运费记录，仅保存交易；确认按钮显示“确认导入 2 笔”，订单数不增加，两笔来源可搜索且无发货。重导不重复；变化须核对，旧版本可见。两次预览依次更新，后者应冲突且可重新核对。检查亮暗主题、密度、键盘、空态、错误重试和只读角色。

### 运费归属验收

使用合成数据验证：多个同名订阅显示疑似；姓名电话地址完整一致且唯一覆盖补费日期才显示高度匹配；跨两目标分配金额合计错误不可保存；预览后修改目标地址须重新核对；退款运费关联不改变主单金额和状态；解除后仍能按旧原文找到来源；只读用户没有关联按钮。

### 来源费用与转投验收

使用合成数据验证退款金额/日期、跨目标退款分配、重复提交、原件更新后净额待核；预览不写入，确认必须核对邮局手续。核对邮局截止与中通起始，拒绝历史/已确认报数/已有发货计划及实发的范围。旧 postal 明细默认 zto 值应兼容，转投后邮局续投不再包含中通目标，进度不能重复累计。

核对未来安排撤回恢复原邮局截止，旧目标与操作记录保留；向后更正新增独立邮局段。管理员可写，普通用户请求返回403；预览后来源、目标或邮局资料变化返回409且无半完成数据。人工检查亮/暗、舒适/紧凑、加载/空/失败/禁用及键盘操作。

### 销售来源统一与跨入口去重（2026-09-22）

使用合成数据：先手工建立微信小程序／CBJ+ 订单，再导入同单号 CBJ 文件，应仅补原件；反向顺序手工新建应被阻止。旧 CBJ小程序／空店铺重导无变化，不能新增订单或原件版本。两个别名订单同号应提示身份冲突；淘宝同号仍独立。确认前增加冲突订单，提交必须失败且无部分写入。

筛选微信小程序，核对列表、补订期、门户和导出合计覆盖两种旧名称；编辑旧导入单时平台及店铺完整。检查亮／暗、舒适／紧凑、空／错误状态。邮局原始渠道不被只读查看改写，VIP 与商学院来源仍可筛选；收款新增历史平台后刷新选项可筛选。中通预览普通付费为个人订阅、赠阅为赠阅，销售来源在备注，签约公司不误填；未知分类冲突阻断。

历史修复用合成重复单验证：未确认主单选择、预览后改资料、来源身份碰撞、重复单已有财务／邮局／发货或运费关联均阻断；在事务中制造失败，身份、状态和关联全部回滚。确认后原件版本不变、旧关联保留、新关联唯一有效、重复单仅作废；同一已完成预览重复执行不再次写入。

## 按具体刊期起订

使用合成订单和隔离数据库，禁止用真实订单保存来验收。

1. 导入促销订阅选一年、中通，改按具体刊期起订：选择 2026-06-08 的刊期，确认开始为当日、结束为 2027-06-07、原成交价不变。
2. 调整结束日到 2027-05-31，确认仍显示一年；保存并重开，模式、期号、日期、价格均不变。恢复自动计算后结束日回到 2027-06-07。
3. 验证新建、草稿重开修改和已生效编辑；已生效需填写变更生效期号；保存失败不留下单独的基础字段修改。
4. 切换年份、无刊期、休刊、请求失败重试、刊期已调整及结束早于开始均有明确反馈；不存在的起投期号和日期冲突不能保存。
5. 中通预览应排除开始前一期和结束后一期；缩短范围排除已有发货关联时应阻断，转投历史不得覆盖。
6. 亮/暗主题、舒适/紧凑密度和键盘操作核对；刊期表未覆盖结束年份时显示待复核，不自动缩短日期；已有历史日期和其他刊物不得重置为月初。
