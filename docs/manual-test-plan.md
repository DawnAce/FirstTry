...
- 步骤：分别准备投诉、收件信息变更、回访工单，电话包含纯数字、空格和连字符三种形式；进入统一列表，依次输入电话号码中间片段，并切换类型、年度和状态筛选。
- 预期：列表使用与新建工单弹窗一致的圆角分区和卡片样式，标题前无装饰图标；每条卡片显示“序号 · 类别”、格式化联系电话、关联信息、登记日期 / 状态和操作，不展示工单内容摘要；电话中间片段均可命中对应工单，空格和连字符不影响搜索，筛选计数与结果同步。
