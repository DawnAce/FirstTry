...
---
**[P1] 高铁运单表区分展示名称与实际收件人**
- 前置：2026-05-18 期存在“高铁展示”发货明细“赵叶 / 15810698235 / 5份”；准备标题含“高铁”的运单工作表，其中 C/E/F/G/H/I 依次为运单号、电话、地址、展示名称、姓名、份数。
- 步骤：进入该期运单核对工作台，上传或拖拽高铁运单文件；若同一文件已有旧草稿，点击「重新上传并解析」。
- 预期：系统读取 H 列“赵叶”为收件人、I 列 `5` 为份数，G 列“商务座候车区（北京站）”只作为展示名称保留；该行进入正常自动匹配，不显示“未识别 / 无效”。
- 为何人工：需要同时核对真实 Excel 列布局、页面展示字段和自动匹配结果，防止展示名称再次错占收件人列。
