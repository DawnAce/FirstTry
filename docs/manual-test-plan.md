...
## 2026-09-10 专项：导入预览、商品关联与批量补订期

以下步骤仅在隔离测试环境使用合成订单。自动化和 Storybook 已覆盖核心行为；部署后的完整页面操作按以下步骤复核，本清单不代表生产验收已完成。

分类按 [导入结果边界](order-import-decision-rules.md) 核对：准备退款且商品已匹配、退款且商品未知、待付款、全为忽略商品、缺日期及混合商品交易，并区分无已有记录、只有业务订单、已有来源原件。确认六类数量之和等于全部；退款加未知商品通常留存，纯运费待付款也留存，已有来源优先判断重复或来源更新，只有业务订单加取消仍跳过。切换筛选后确认范围仍是整批导入/留存/来源更新；漏勾来源更新须阻断整批且不写入。确认后只有导入行建单，留存保存来源，更新保留新旧原件，原订单资料和状态不被重导修改。分别核对预览重复数和确认时新增发现的重复数。
...
6. **预览分类筛选与订期展示**：准备超过 50 单的混合结果，让重复和待确认订单位于原列表第二页。先补期号/期次，再切换「待确认」「重复」「跳过」「导入」，检查筛选覆盖整份预览、回到第一页、已补内容保留，分类计数始终为整批数量。重新识别后保持当前分类并刷新数量，结果为零时明确显示空态；更换文件或设置后回到「全部」。在「重复」分类下确认导入，仍应按按钮所示数量处理整批可导入订单。列表和识别详情的订期须显示起止日期，缺失日期显示「未填写」；键盘可切换分类，选中标签在亮暗主题中均清晰可辨。
...
用合成 Excel 上传已发货及已退款纯运费各一笔：预览为留存，确认后订单数不增加，两筆来源可搜索且无发货。重导不重复；变化须核对，旧版本可见。两次预览依次更新，后者应冲突且可重新核对。检查亮暗主题、密度、键盘、空态、错误重试和只读角色。
