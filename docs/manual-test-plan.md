...
---
**[P1] 发货状态卡和运单确认栏按钮文字对比度**
- 前置：分别准备“实际发货仍有待处理”“实际发货接口失败”和“运单导入存在已匹配行”的页面状态，并同时核对亮色、暗色主题。
- 步骤：查看单期快递管理中的「继续处理」「重新加载」按钮，以及运单工作台底部「导入已核销的 N 份」主按钮。
- 预期：状态卡说明文字继续使用次要文字色，但按钮内部文字不继承说明文字样式；「继续处理」「重新加载」文字清晰可读，蓝色主按钮文字保持白色。状态卡与确认栏的标题、说明仍按原布局分行显示。
- 为何人工：需要核对 CSS 后代选择器不会误伤 Ant Design 按钮内部元素，并检查两种主题下的实际视觉对比度；Storybook 交互测试覆盖关键文字颜色。
