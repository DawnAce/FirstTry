...
- 前置：2026-01-26 期确认印数 1,321 份；确认版发货计划含“肖波 / 成都杂志铺 / 365份”，另有采用每月两次合寄、当前暂停寄送的 1 份。运单源文件共 1,320 份，其中 955 份可自动匹配，肖波的 365 份拆为 65、100、100、100 四个运单且初始未匹配，暂停寄送的 1 份不在源文件中。
- 步骤：
  1. 上传文件进入运单核对工作台，检查顶部两条状态和待处理列表。
  2. 先确认 955 份已匹配运单，再次进入同一批次。
  3. 选择四条肖波运单，批量关联到“成都杂志铺 / 365份”计划明细。
  4. 在“文件未覆盖”中登记 1 份“无需发货”，原因填写“每月两次合寄 · 暂停寄送”。
- 预期：首次预览显示源文件 1,320 份、已关联 955 份、待人工关联 365 份、文件未覆盖 1 份；确认 955 份后四条未匹配运单仍完整可见且可编辑。批量关联后生成四个独立包裹，合计 365 份；登记暂停寄送后实际核销为“运单 1,320 + 无需发货 1 = 1,321”，待补归零。“文件未覆盖”处理结果完整显示“每月两次合寄 · 暂停寄送 1份”。上方“发货计划对账”始终只比较确认时计划与当前计划，不因实际核销进度改变。
