...
---
**[P0] 1,321 份确认印数：955 份自动匹配 + 365 份拆单人工关联 + 1 份暂停寄送核销**（运单未匹配不得丢失）
- 前置：2026-01-26 期确认印数和确认时计划均为 1,321 份；确认后每月两次合寄的 1 份暂停寄送，当前计划变为 1,320 份。当前计划含“肖波 / 成都杂志铺 / 365份”。运单源文件共 1,320 份，其中 955 份可自动匹配，肖波的 365 份拆为 65、100、100、100 四个运单且初始未匹配，暂停寄送的 1 份不在源文件中。
- 步骤：
  1. 上传文件进入运单核对工作台，检查顶部两条状态和待处理列表。
  2. 先确认 955 份已匹配运单，再次进入同一批次。
  3. 选择四条肖波运单，批量关联到“成都杂志铺 / 365份”计划明细。
  4. 在“文件未覆盖”中登记 1 份“无需发货”，选择实际暂停寄送的收件明细，原因填写“每月两次合寄 · 暂停寄送”。
  5. 再准备一条历史无 `shipping_detail_id` 的无需发货记录，确认页面显示“待补充归属”，随后在工作台补选对应明细。
- 预期：首次预览显示源文件 1,320 份、已关联 955 份、待人工关联 365 份、文件未覆盖 1 份；确认 955 份后四条未匹配运单仍完整可见且可编辑。批量关联后生成四个独立包裹，合计 365 份。登记并归属暂停寄送后顶部显示“计划已对平”，公式为“1,321 = 当前计划 1,320 + 已归因停发 1”；实际状态显示“核销已完成 · 部分发货”，实际发出 1,320、无需发货 1、核销待补 0。“查看变更与归因”能看到收件人、渠道、原因和份数。历史无归属记录在补齐前不对平计划，补齐后才变为已归因。
