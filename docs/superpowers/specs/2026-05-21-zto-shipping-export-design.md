...
用户在「收件人管理 → 中通发货明细」维护某期中通发货明细时，需要直接导出该期的中通发货明细 Excel。目前后端已经提供按 issue 导出的 `/api/issues/{issue_id}/export/shipping` 接口，历史页也能通过「导出全部」间接导出，但中通明细维护页缺少直接入口。

## 范围

- 导出当前选择期号对应的全部中通发货明细。
- 导出不受页面筛选条件、搜索关键字或表格勾选状态影响。
- 复用现有中通发货明细 Excel 模板、文件命名和导出快照逻辑。
- 不新增筛选导出、勾选导出或新的 Excel 格式。

## 推荐方案

在「中通发货明细」筛选面板底部右侧、现有记录汇总和「新增」按钮附近增加「导出」按钮。按钮读取当前选中的 `Issue`，调用现有导出地址 `/api/issues/{issue_id}/export/shipping`，由浏览器触发文件下载。
