...
- **纯后端、零迁移、无前端改动**。测试 +1:operator 命中敏感端点 403、日常操作放行。后端 390→391。
- 遗留(F 之外,可后续):`exports.py` 的印数/发货导出路由仍仅靠路由级登录、未加 require_admin;前端未按角色隐藏管理员按钮(operator 点了会收 403 报错,功能安全但 UX 可优化)。
