...
- **运费补拍/转中通**：同一可导入订阅交易的运费计入总额，不建刊物明细；含中通行时提示核对投递改中通。独立纯运费只留存，关联订阅和转投分别确认，不自动改主订单投递。
...
- **订单状态**：建单资格排除待付款/取消，退款继续校验；来源阶段可将纯运费、未通过建单校验的退款归留存，已有来源优先判断重复/更新。未知状态默认已付款并标记未知，核对后确认；来源更新不回写主订单状态。
...
- 来源关联已完成：候选订阅、跨目标金额分配、人工预览确认、历史关联和原文搜索。同批订阅确认入库后可成为运费候选。
