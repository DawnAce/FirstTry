...
> 最后更新：2026-09-10。围绕「把 CBJ 小程序 / 淘宝 等电商订单导入订单管理系统 + 下游统计」这条线。
> **状态：CBJ + 淘宝 端到端完成、已合并 main、已部署生产；商品库已规范化（三段式命名 + 结构化 code，生产 13 个稳定品）；新增「商学院按期发行量（单期 + 订阅）」并部署。2026-06-27 修了订单管理 5 个正确性 bug（见「正确性修复」节）+ 建了「退款闭环」（见同名节）+「按期批量排发」（见同名节）+「已发货回写 + 对账」（见同名节）+「收款 / 欠款追踪」（见同名节）+「列表增强：搜索/排序/批量/导出」（见同名节）+「端点权限 F」（见同名节）；2026-09-07 补回订单列表的「电商导入」入口并增加角色显隐回归测试。后端 391 测试全绿、前端 `tsc -b && vite build` 通过。**

## 2026-09-10：导入预览、批量补订期与关联已有商品

- **预览结果筛选**：原静态计数标签改为全部、待确认、重复、导入、跳过筛选按钮，支持键盘操作、选中状态、分类空态和筛选后分页；切换分类保留已补信息，重新识别后刷新当前分类。确认导入范围仍为整批可导入订单，仅涉及前端展示。
- **订期展示与筛选验证**：列表和识别详情统一显示「订期：开始日期 至 结束日期」，缺失日期明确显示「未填写」。导入页 6 项 Storybook 交互测试、168 项前端单测及应用构建通过，Lint 无错误（54 条既有警告）；已用合成订单核对亮色、暗色紧凑及空结果界面。README、需求、用户指南和人工验收步骤同步更新。
- **关联已有商品**：待确认商品可选择现有商品、核对履约属性后追加完整名称为别名并重新预览；同款促销无需新增商品，参考价不覆盖实际成交价。保留原别名、重复追加幂等、拦截停用及重名冲突。合成 17 单验证了关联、重新识别、正式导入全流程，商品数量与实际成交价保持不变。
- **批量补订期**：导入预览内补录与订单管理待补队列共用窗口，支持明细筛选、跨页选择、自然月/实际日期填写、预览与原子确认，保护价格与既有日期并保留审计。导入中的补录仍需确认导入才建单；重新识别前会提示清除已补日期的影响。另修复导入订单在普通编辑页补日期时回填标准套餐价的问题。
- **验证与交付范围**：后端全量 736 通过、2 项因缺少历史样本跳过；前端单测 168 通过，商品关联相关 Storybook 17 通过；应用与 Storybook 构建通过，Lint 无错误（54 条既有警告）。无数据库迁移，本次交付不包含生产部署或真实商品、订单的数据操作。
...
| 4i（2026-09-07）| **订单列表导入入口修复**：在 `/orders` 右上角补回「电商导入」按钮，跳转 `/orders/import`；沿用 `canMutate` 权限，演示只读角色隐藏。Storybook 覆盖可编辑角色可见与只读角色不可见。| —（前端）|
...
之后：**商品管理（商品库）/ 订单管理 → 电商导入**，传 Excel → 预览 → 确认导入。
...
## 待办 / 后续
- **真导一批（CBJ / 淘宝）**：基础导入代码 + 商品库（13 品）+ 刊历已部署生产；干跑预览验证过（CBJ 94/6、淘宝 36/6/1）。2026-09-10 增强功能部署后，操作流程为：传 Excel → 预览 → 关联待确认商品 → 批量补订期 → 确认导入；也可导入后从「待补订期」分批补齐，再核对收件人并同步中通。淘宝遗留：多商品单单价按标题均摊（需人工核拆）；商家备注里起止期未自动解析（批次起投月 + 明细核对）；季度用 custom 建模。
...
- **预览内编辑**：缺失订期已可在「批量补订期」窗口逐行调整并应用到导入预览；已有完整订期、订单状态及收件人修改仍在订单详情处理。
- **覆盖期算法 `coverage_rule` 暂不动（2026-06-24 选 C）**：它只在**电商导入**时生效（`cbj_order_import_service._coverage_for`），给订单行算覆盖期（起止日期）；手工新建/确认都不经过它。只有「按起投月算」`term_from_month` 真正算（批次起投月 + 时长），订阅行恒用它；「最新一期」/「自定义」导入时留空。⚠️ **「固定日期」`explicit` 目前没接上**——`_coverage_for` 对它返回空，商品上那对固定起止日期从不被读取，选了≈自定义留空（schema 仍强校验那对日期，但无人消费）。以后清理进展：商品表单「覆盖期算法」下拉已**去掉「固定日期(explicit)」废选项**（PR #43，UI 不可选；后端枚举与 `_coverage_for` 逻辑不动，选它本会 422）。剩余可选清理：按「履约类型」自动带默认。
...
- `GET /api/order-coverage/candidates`、`POST /api/order-coverage/preview`、`POST /api/order-coverage/apply` → 管理员批量补订期（导入会话或已入库订单）
- `POST /api/products/{id}/aliases`（alias）→ 向现有启用商品追加别名，不修改价格或新建商品
