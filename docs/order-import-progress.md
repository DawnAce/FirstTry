...
| Phase 2.5 | **商品库** `products` 表 + 管理 CRUD API + seed（3 个 CBJ 商品）| ✅ 已建表 |
...
**后端 281 测试、前端 75 测试全绿；`npm run build` 通过；真实 CBJ 文件预览验证通过。**
...
3. **seed 商品库**：用管理员登录后调一次 `POST /api/admin/seed`（会加那 3 个 CBJ 商品；之后在 `/products` 页维护）。

之后：**订单管理 → 商品库 / 电商导入**，传 Excel → 预览 → 确认导入。

## 待办 / 后续
- **上生产试导一批**：seed 后用真实文件真导一批验证。
