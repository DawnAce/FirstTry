...
## 淘宝平台导入（阶段 1，已完成 · 待提交/部署）
> 2026-06-25。把淘宝订单融进同一套订单管理（同表 / 同列表 / 同统计 / 同发货同步）。
...
- **淘宝上线**：阶段 1 代码完成、**未 commit/未部署**；上生产前 `POST /api/admin/seed`（现会跑 `sync_catalog` 补淘宝别名/商品）→ 传淘宝 Excel 干跑预览 → 真导。后续：多商品单单价按标题均摊（需人工核拆）；商家备注里的起止期未自动解析（沿用批次起投月 + 逐单核）；季度未单独建 SubscriptionTerm（用 custom + custom 覆盖期）。
