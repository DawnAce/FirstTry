...
> **状态：CBJ + 淘宝 端到端完成、已合并 main、已部署生产；商品库已规范化（三段式命名 + 结构化 code，生产 13 个稳定品）；新增「商学院按期发行量（单期 + 订阅）」并部署。2026-06-27 修了订单管理 5 个正确性 bug（见「正确性修复」节）+ 建了「退款闭环」（见同名节）+「按期批量排发」（见同名节）+「已发货回写 + 对账」（见同名节）。后端 385 测试全绿、前端 `tsc -b && vite build` 通过。**
