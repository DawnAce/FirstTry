...
> 最后更新：2026-09-22。围绕「把 CBJ 小程序 / 淘宝 等电商订单导入订单管理系统 + 下游统计」这条线。
> **状态：CBJ + 淘宝核心导入流程、商品库、来源交易和后续履约功能已完成。各日期条目记录对应交付的功能、测试和部署范围；最新修复及验证结果见下方 2026-09-22 条目。历史部署记录不代表本次修改已经部署，合并 main 后仍需单独更新运行环境。**

## 2026-09-22：确认导入超时与提交后误报修复

- 确认整批导入原先继承普通请求的 120 秒超时，后端仍可能继续执行，页面却只显示“导入失败”。确认请求改为等待服务器结果，保存期间显示提示并禁止重复提交；连接中断时明确提示核对结果，保留文件和预览，重新预览成功后解除旧错误。
- 后端改为提交前提取结果 ID，避免提交后 ORM 过期引发逐单回读，以及已落库后回读失败被误报为导入失败。事务、权限、核对、审计和去重规则保持不变，无数据库迁移。
- 回归使用合成 Excel 和 SQLite，覆盖 129 单订单＋2 笔运费、提交后数据库不可读、重新预览去重及前端超过两分钟的等待；不重放用户真实导入。
- 验证：后端 897 项通过、3 项因本地样本或 CI MySQL 条件跳过；前端 183 项单测、39 项导入页面交互通过；Lint 0 错误（54 条既有警告），应用构建通过。电脑控制工具未能返回测试页截图，未完成人工亮暗主题视觉复核；运行数据核对另以只读事务进行，真实批次明细不写入仓库。未部署、未修改业务库数据。

## 2026-09-22：导入预览视觉简化

- 固定保留「全部、待确认、来源更新、重复、可导入、跳过」六个文字筛选入口，数量为 0 也显示并可进入空结果，选中项使用下划线；不将「待确认」与识别核对合并。
- 来源交易、刷新草稿、批量补订期和运费关联移到上方工具区；期号与识别共用一条必核提示，入口显示各自待核数量。识别筛选保留「处理下一条」，辅助筛选提供「返回全部」。批次摘要置于标题下，一般说明改为普通文字。
- 分类、计数、核对和整批确认规则保持原逻辑，无后端或数据库变更。验证：前端 178 项单测、37 项导入 Storybook 交互通过；应用及 Storybook 构建通过，Lint 0 错误、54 条既有警告。以合成数据核对亮/暗主题、舒适/紧凑密度、375px 窄屏、零结果、只读及刷新状态；未执行真实订单导入或部署。

## 2026-09-11：识别核对与导入前运费关联

- 列表与详情提供“核对／修改识别”，可更正本单投递、真实交易状态、金额分摊、下单日期及商品。原生中通不再误报转投；冲突按明细核对，未知状态不默认已付款，负数或金额不符不能只确认警告后放行。顶部可筛选待核对并处理下一条，往期期号及商学院期次在列表和详情共用输入。
- 核对保存在有版本的服务端草稿，修改后重算本单；投递更正保留人工补录订期并要求重新确认，商品更正清除依赖旧明细的核对。原件、原实付不改写，正式导入原子写入并记录依据与前后值。草稿、商品配置、刊期、关联目标变化均有冲突检测。旧来源指纹兼容派生状态规则调整，原件未变仍判重复。
- 独立运费预览可选择已有或本批订阅收件目标、分配金额，与本批订单一起保存；推荐综合收件资料、订期及唯一性，不自动关联。找不到可留存待处理，导入结果逐笔给出继续关联或核对投递入口。关联不自动转中通，仍须另行核对正式生效期和邮局截止。
- 验证：后端全量 877 项通过，3 项因本地样本或 CI MySQL 条件跳过；前端单测 174 项、导入及来源相关交互 41 项通过；Lint 0 错误、54 条既有警告，应用与 Storybook 构建通过。合成数据人工核对两种弹窗的亮/暗、舒适/紧凑及 375px 窄屏，页面与弹窗无横向溢出，候选表可内部横向滚动。专项覆盖权限、过期、目标变化、失败回滚及重试。
- 无数据库迁移；代码、测试和文档通过独立分支及 PR 交付。运行环境更新后须刷新页面并重新上传预览；本次不包含服务器部署或真实业务订单验收。下列版本的部署状态为历史记录。

## 2026-09-11：翻期临界期号核对

- 预览和订单详情提供「确认第 XXXX 期 / 修改期号」，改选时显示刊期表的期号和出版日期，一单多条单期明细分别确认。「期号待核对」辅助筛选可跨分页定位，保留原五类结果计数；修改选择撤销该明细核对，新预览清除旧结果并提前提示。
- 最新一期的临界、缺付款时间或刊期范围不足等不确定明细须显式确认；前后端均阻断漏核对的整批提交。后端按缓存明细校验，复核刊期表唯一、非休刊及日期快照，在同一事务保存订单、来源和 `import_issue_review` 审计。原始来源、实际价格和往期/商学院补期流程保持原规则；无数据库迁移。
- 验证：后端全量 859 项通过，3 项因缺少本地黄金样本或仅限 CI MySQL 跳过；前端单测 174 项、导入页交互 23 项通过，Lint 0 错误（54 条既有警告），应用及 Storybook 构建通过。合成数据检查亮/暗主题与舒适/紧凑四种组合及 375px 详情，均无横向溢出。
- 交付范围为代码与文档，不包含服务器部署或真实订单写入；无数据库迁移，运行环境更新代码后须刷新页面并重新预览。下方历史版本的部署记录不代表本次变更已部署。

## 2026-09-11：导入详情紧凑布局与中文展示

- 详情抽屉调整为 640px，交易、收件、商品信息分组展示；字段名固定宽度横排，普通行舒适模式 40px、紧凑模式 36px，长地址和原表内容在值区域换行。开票及其他信息、原表信息可展开，文件和行号保留。
- 使用前端展示转换替代原始对象序列化：金额、日期、平台状态及原表字段均可直接阅读；主列表同步中文状态与可操作的商品/日期提示，未知状态保持待核对。
- 来源更新逐项展示修改、新增、移除、清空及格式变化，包含原表独有备注、商品属性等内容。原件保存和后台判定不变；只读账号、请求处理中或缺少前后详情时不能勾选。
- 验证：前端全量单测 174 项、导入页交互 17 项通过；Lint 0 错误（54 条既有警告），应用与 Storybook 构建通过。合成数据已核对亮/暗主题与舒适/紧凑四种组合，以及 375px 窄屏，字段名高度 22px，详情无横向溢出；已查看来源变化页面。未操作真实订单或生产数据库。

## 2026-09-11：合并可导入分类与退款待确认

- 取消独立“留存”筛选；后台 `import` 与 `retain` 统一展示为“可导入”，按笔合计。纯运费和旧订单补原件在行内注明“仅保存交易”，摘要分别列出新建订单数和交易记录数。
- 新退款/部分退款订单商品未知、缺下单/支付日期或无商品时保留“待确认”和具体原因，可以就地关联商品、重新识别。待确认不提前写入来源；若先导入同批其他记录，保留原文件，补齐后再次上传仍可建单。
- 原件保存、来源更新逐笔确认、去重、事务及审计继续沿用。API 返回结构、数据库及旧版已保存来源不变；现有旧来源重新上传仍按重复/来源更新处理。
- 验证：后端全量 832 项通过，3 项因缺少本地黄金样本或仅限 CI MySQL 跳过；前端单测 168 项、导入页交互 11 项通过（与邮局补日期 9 项合跑，共 20 项通过），Lint 0 错误（54 条既有警告），应用与 Storybook 构建通过。使用合成数据核对亮色舒适、暗色紧凑页面；未操作真实订单，未执行生产导入。
- 以下 2026-09-10 六类分类记录为当时交付历史，当前规则以本节和 [分类边界](order-import-decision-rules.md) 为准。

## 2026-09-10：导入预览、批量补订期与关联已有商品

- **升级后预览失败排查**：来源交易表尚未迁移会导致预览 500；代码热更新不会应用迁移。新增来源缺表/缺列的 503 说明、页面持久错误与重试提示，预览失败时阻止确认旧结果；使用合成文件覆盖缺表、缺列、正常预览确认和异常后重试。
  - 相关后端回归 64 项、前端单测 168 项、导入页浏览器交互 8 项通过；构建通过，Lint 无错误（54 条既有警告），亮色舒适/暗色紧凑提示已核对。当前业务库经授权补齐两条来源迁移，原文件预览恢复，未执行真实订单确认导入。

- **分类边界核验**：新增 [分类规则说明](order-import-decision-rules.md)，按已合并的来源交易功能核对“建单资格 → 来源交易”两阶段及六类最终结果。页面共享说明和整批确认范围同步更新；合成回归覆盖退款留存、来源更新优先级、原件不变重复、平台店铺范围及 Excel 预览→确认，确认原业务订单不被重导修改。本地专项 44 项、相关后端回归合计 98 项、前端单测 168 项及导入页交互 6 项通过，构建成功；Lint 0 错误、54 条既有警告，亮/暗及紧凑界面无文字溢出。未改分类算法，未核对具体真实退款订单或操作生产数据。
...
| 4g | **「最新一期」自动判期号与人工核对**：导入时 `latest_issue` 单期行按“付款时间 + 刊期表 + 周五约 22 点翻期”给出建议。翻期点 ±4h 的临界单、缺付款时间或刊期范围不足的明细，须逐条确认或从刊期表中改选实际期号；全部核对后才能导入，结果与订单一并写入审计。预览表和详情共用核对入口，提供「期号待核对」筛选。无迁移。| —（前后端）|
...
以下按当前来源交易功能更新；此前各阶段的测试数量和分类结果是当时版本的记录。当前完整判定顺序以 [分类边界](order-import-decision-rules.md) 为准。
...
- **运费补拍/转中通**：同一可导入订阅交易的运费计入总额，不建刊物明细；投递冲突逐明细确认后重算订期，原生中通商品不误报转投。独立纯运费只留存，关联订阅和转投分别确认，不自动改主订单投递。
- **运费识别**：产品名含「运费」或「快递费」视为运费行；纯运费在无已有来源时归留存，包括待付款、取消或缺日期，后续另行核对费用和关联。
- **忽略名单**：家族企业、深潜、深度系列、电子刊、对公专用、利润薄如刀片。多商品单仅移除命中行；整单忽略还受日期、已有订单和来源检查的优先级影响，不是无条件跳过。
- **订单状态**：建单资格排除待付款/取消，退款继续校验；来源阶段可将纯运费归留存，未通过建单校验的新退款仍待确认，已有来源优先判断重复/更新。未知状态保持未判定，选择实际状态后才能确认；来源更新不回写主订单状态。
- **历史归档模式**：保留下单日期，订期可补填可留空（不自动估算），打归档标记、**默认不自动同步**（不被硬拦，补齐后可手动同步）、列表可单独筛。
- **去重**：来源按平台、店铺和单号识别，不变为重复、变化为来源更新。只有业务订单且尚无来源原件时，通常补留原件；不重复建单。
...
- **预览内编辑**：缺失订期已可在「批量补订期」窗口逐行调整并应用到导入预览；本单状态、日期、商品、投递及分摊可在核对窗口更正；已有完整订期及收件人仍在订单详情处理。
...
- 来源关联已完成：候选订阅、跨目标金额分配、人工预览确认、历史关联和原文搜索。导入预览已支持已有及同批待导入订阅候选，最终确认时原子建立关联。
- 来源费用及转投已完成：累计退款核对、多目标退款分配、净收汇总、正式刊期及邮局手续单独确认、撤回和向后更正。邮局续投和履约进度适配转投边界，原件与所有核对操作保留审计。

## 销售来源口径修复（2026-09-22）

CBJ 导入统一为微信小程序／CBJ+，与手工录入一致；旧 CBJ 名称和空店铺通过身份规则兼容。预览、提交和原件关联共同核对平台／店铺／单号，已有唯一订单只补原件，多个匹配阻断。列表、补订期、统计、导出使用同一平台口径。历史原始版本不改写；存量重复订单由单独修复预览核对。

## 2026-09-22 导入订单补录具体起投刊期

普通编辑页可保留一年／半年期限，改按中国经营报正式刊期起订并调整结束日；导入成交价原样保留。草稿和生效订单的明细均可保存；旧数据只在人工保存时记录起投模式，本次迁移不补写任何订单日期。批量补订期仍沿用现有月份／实际日期流程。
