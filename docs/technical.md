...
- `OrderDetail.tsx` 的下方信息区统一为 6 个标签页：订单内容、履约方案、收款记录、关联快递、关联邮局、事件流。标签使用图标和数量徽标；履约方案改为版本工单卡片，投递方式与目标份数使用响应式横向配置条；收款、邮局关联和事件流分别使用摘要流水卡、投递工单卡与纵向时间线。邮局订单的快递标签保留入口但展示“不适用”空状态，原有同步和信息变更业务动作不变。
