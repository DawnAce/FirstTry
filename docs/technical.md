...
- Preview：`GET /api/orders/{order_id}/shipping-sync/preview?issue_number=2625` 返回 `OrderShippingSyncPreview`，只预览 active 订单在指定期号的每个 active 明细 + 履约目标将创建、更新、跳过或冲突的结果，不修改数据。
