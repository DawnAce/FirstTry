...
- **`source_type` 字段的录入方式收敛**：原 5 个枚举值（`ecommerce` / `corporate_transfer` / `vip_gift` / `manual` / `mail_annual`）实际混杂了 4 个维度的概念（销售渠道 / 付款方式 / 业务性质 / 录入方式），与已有的 `source_platform`（销售渠道）/ `payment_method`（付款方式）/ `billing_type`（业务性质）字段重复。**PR-A 已将 UX 解耦为"录入方式"** —— 前端表单完全隐藏该字段，列表筛选器移除，详情页用 Tag「📥 手工录入」展示；后端 `OrderCreate.source_type` 默认 `manual`、`OrderUpdate` 完全移除该字段（provenance 元数据任何状态下都不可改）。数据迁移 `d8a1f4e7b9c2` 已把全部历史数据规范化为 `manual`。V1.3 电商批量导入 / API 同步启用前，建议继续推进 DB schema rename `source_type` → `entry_method`，枚举值收敛为 `manual / excel_import / api_sync`
...
- **期刊表**（`/schedule`，`ScheduleView.tsx`）：使用 `GET /api/schedule` 按年份展示出版期数、休刊次数、期号范围等概览统计，支持按月份、日期、期号、状态（正常/休刊）筛选，按月份分组展示刊期明细。数据通过 TanStack Query 以 `['schedule', year]` 缓存。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/orders` | 分页列表，支持 `status` / `source_type` / `payer_name_like` / `coverage_start` / `coverage_end` / `has_drift` 筛选，返回 `{ rows, total }`（`source_type` API 参数保留以备 V1.3 批量导入 / API 同步扩展，前端 UI 不再暴露筛选器，详见 §3.15 录入方式说明） |
...
| `POST` | `/api/orders/{id}/void` | 作废订单（任何状态都可），body `{ reason }` 必填 |
| `GET` | `/api/orders/{id}/shipping-sync/preview?issue_number={issue}` | 预览 active 订单在指定期号将创建/更新/跳过/冲突的 `shipping_details` 行 |
| `POST` | `/api/orders/{id}/shipping-sync/apply` | 按 body `{ "issue_number": 2625 }` 写入订单生成的 `shipping_details` 行；有冲突时返回 409 |
...
- 输入：`fulfillment_type` / `coverage_start` / `coverage_end` / `issue_number`
- 算法：根据出版物在 `publication_schedule` 中按周排期 + 跳过 `is_suspended` 的休刊期数；订阅按覆盖范围计算落在该范围内的实际期数；单期固定为 1
...
- Preview：`GET /api/orders/{id}/shipping-sync/preview?issue_number=2625` 返回 `OrderShippingSyncPreview`，汇总候选、待创建、待更新、跳过和冲突数量。
- Apply：`POST /api/orders/{id}/shipping-sync/apply` 使用同一预览逻辑，写入或更新订单生成的 `shipping_details`，并记录 `synced_to_shipping` / `shipping_sync_conflict` 事件。
- 进度：订单详情与 `/fulfillment-progress` 中的 `synced_count` 直接统计已关联到 `order_id + order_item_id` 的发货明细行；`skipped_count` 暂保留为 0。
