...
| `POST` | `/imports/{batch_id}/rows/{row_id}/warehouse-stock-in` | 将误标为无需运单的“马飞—库房留存”导入行原子转换为库存入库；原行保留为已忽略凭证，并将同明细已有无需发货核销改按库存入库分类 |
