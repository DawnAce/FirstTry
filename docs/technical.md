...
V1.1 新增独立的订单管理子模块，与刊期 / 印数 / 物流子系统正交：

| 表 | 说明 |
|----|------|
| `orders` | 订单主体（订单编码、付款主体、来源、状态、金额）。状态机：`draft → active → void`（`pending_confirmation` 为 V1.2 预留状态，V1.1 不会自动跳入）|
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/orders` | 分页列表，支持 `status` / `source_type` / `payer_name_like` / `coverage_start` / `coverage_end` / `has_drift` 筛选，返回 `{ rows, total }`（V1.1：`source_type` API 参数保留以备 V1.2 扩展，前端 UI 不再暴露筛选器，详见 §3.15 录入方式说明） |
...
- [ ] 订单管理 V1.2（明细在 active 状态下就地编辑、多版本 allocation、**渠道订单 + 收款流水 + 欠款追踪 + 未付清筛选**）
- [ ] 订单 → 发货同步流水
- [ ] 数据统计与报表分析
- [ ] 自动发送邮件通知
- [ ] 移动端适配
