...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/orders` | 分页列表，支持 `status` / `source_type` / `payer_name_like` / `coverage_start` / `coverage_end` / `has_drift` 筛选，返回 `{ rows, total }`（`source_type` API 参数保留以备后续版本批量导入 / API 同步扩展，前端 UI 不再暴露筛选器，详见 §3.15 录入方式说明） |
