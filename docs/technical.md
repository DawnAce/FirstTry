...
### 3.12 issue_audit_snapshots（确认/导出快照）
...
|-------|------|
| `基本信息` | 期号、出版日期 |
| `发货明细` | 20列字段（工作表名称、渠道、子渠道、运输方式、频次、状态、姓名、地址、电话、数量、截止日期、备注、附加信息、城市、网点名称、网点大厅、联系人、序号、期数、公司） |
...
- 原始印数表会校验原表总印数与映射后总数一致；存在未处理临时加印、未命中映射项或总数不一致时，预览会返回错误且不生成导入会话
- 原始中通多工作表文件从标题中的“总第XXXX期”识别期号；月底多期标题取最大期号作为本次导入期号
- 原始中通导入时，`马飞` 且运输方式为 `库房留存`、行内信息包含 `中通库房` 的记录，城市固定补为 `北京`
- 目标期号不能已存在（防止重复导入）
...
校验失败时 `can_commit` 为 `false`，`errors` 包含具体错误信息，`import_session_id` 为空字符串。
...
**字段**：序号、期号、原工作表、渠道、子渠道、签约公司、姓名、电话、地址、份数、频率、运输方式、发货时间、截止日期、状态、备注、附加信息、城市、站点、站厅、联系人、高铁序号、期数、信息确认。
