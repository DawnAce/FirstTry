...
| 订单详情 | `/orders/:id` | 邮局工单风格的摘要、履约进度与 6 个图标化标签页；履约方案采用版本工单卡，收款/快递/邮局/事件流采用统一卡片和时间线。邮局按刊期表已到出刊日的期数计算履约进度，收报人可发起居中弹窗式信息变更，并展示处理中、已变更次数与历史工单 |
...
| 商品库 | `/products` | 商品 CRUD + 命名规则（已移至顶级侧边栏菜单「商品管理」，不再在订单管理子菜单下）|
