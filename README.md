...
每周五生成下周一出版的《中国经营报》印数管理表和中通快递发货明细的 Web 应用，并支持管理员上传年度刊期 PDF，预览校验后更新系统刊期表。报数编辑页支持把北京邮发、北京报零、广州日报和成都杂志铺的 PDF / 图片作为原始凭证归档，经 OCR 识别与人工确认后写入草稿数据；成都跨月补发独立计入结算与补发执行，不回写已经出刊的印数。系统从 V1.1 起还包含**订单管理**模块，V1.3 已支持手工录入、订阅套餐价、active 状态明细编辑、多版本履约方案，以及单订单按期号手动预览 / 应用同步到 `shipping_details`。并已上线**电商订单导入**（CBJ 小程序 + 淘宝，上传按表头自动识别平台）与**商品库**：商品库把电商商品名映射为履约属性（名称三段式规范、与匹配解耦——靠别名匹配，改名不影响识别），导入页支持上传 Excel → 预览（自动识别商品/状态/运费转中通/套餐拆分，并判断来源留存和更新，结果标签按完整规则确定）→ 确认批量建单，含「近期 / 历史归档」两种模式。还提供**活动订单统计**（按活动 / 按期）与**商学院按期发行量**（单期 + 订阅展开）。
...
- **邮局信息变更**：已应用工单可由管理员为已明确收件人的去向补齐缺失起投日期，自动带入收件信息并记录审计；已有日期不能覆盖，待确认份数继续保留。操作步骤见 [用户指南](docs/user-guide.md#291-邮局工单投诉--信息变更--回访-三合一)。
- **电商订单导入（CBJ 小程序 + 淘宝）**：上传按表头自动识别平台 → 商品库映射 + Excel 批量导入（预览/确认）+ 关联已有商品或快速新增商品 + 活动标签/赠品（按活动追溯统计）。预览可按全部、待确认、重复、可导入、来源更新、跳过筛选；纯运费和已有订单补原件归可导入，行内注明仅保存交易；新退款订单识别失败进入待确认。详情以紧凑中文条目分组展示交易、收件和商品信息，开票及原表信息可展开，来源变化逐项对比。筛选保留已补信息，确认仍处理整批可导入记录/来源更新，更新须逐笔核对。五类标签与退款例外见 [分类边界](docs/order-import-decision-rules.md)。同款促销可追加别名，保留实际成交价；导入预览和订单列表支持**批量补订期**，可筛选、多选、核对后分批补齐缺失日期，并保留审计。商品库采用三段式命名 + 结构化 code（名称与匹配解耦）；**商学院按期发行量**统计包含单期与订阅覆盖期展开、合刊去重。操作步骤见 [用户指南](docs/user-guide.md#2d4-批量补订期)，交付记录见 [进度备忘](docs/order-import-progress.md)。后续重点：财务对账、客户自助下单、其它平台
...
- **期刊表**（`/schedule`）：按年份查看全年计划、已出刊/未出刊进度、休刊次数、期号范围和版数调整，支持按月份、日期、期号、出刊状态筛选。全年矩阵按北京时间出版日期区分淡绿色的已出刊、普通底色的未出刊，并突出本年下一期。历史年份只有刊期计划时明确显示“已出刊（按刊期）”“计划版数”“未录入印数”；没有可对比版数时显示“—”，不把计划值当成实际值。
...
- 印数确认前，同渠道后续文件必须明确选择「追加」或对某一文件执行「重新上传文件」：追加会计入有效来源合计，重新上传只替换所选文件的贡献，其他来源不变。原文件正确但人工核对数字填错时，可直接点「更正核对数字」建立新核对版本，无需重复上传；旧记录保留为「已替换」凭证。
...
> **限制**：报数文件需使用系统模板或可识别的原始报数表；原始报数表如存在未处理临时加印、总印数不一致或未命中映射项，会在预览阶段阻断；中通发货文件支持系统单表模板和原始多工作表格式，子渠道仅保留“监管 / 政府”，其他历史说明会移入备注并在预览中提示；所有0份发货行直接忽略并提示，不生成计划占位；两份文件必须属于同一期且目标期号不能已存在；报数中的“中通物流公司”合计必须与中通发货明细数量一致；导入成功后沿用现有确认、发货、导出流程。
...
发货计划中的“停发”与实际发货中的“无需发货/停发归因”使用同一业务链：设为停发会自动登记同明细、同份数的无需发货归因，恢复正常会撤销系统自动归因；页面分别显示计划总数、仍需寄出的当前计划和停发份数。
...
## 文档
- [Codex 项目规则](AGENTS.md) — 仓库范围、业务不变量、开发规范、安全边界与交付要求
...
电商导入支持纯运费记录和已识别退款订单原件保存、关联订阅、跨原件搜索、独立费用核对及按刊期确认转投。入口为订单管理的“来源交易”，财务工作台也可核对运费。部署需执行迁移 `a7c9e1f3b5d8`、`b8d0f2a4c6e9`；流程和边界见 [实施文档](docs/order-source-transactions.md)。
