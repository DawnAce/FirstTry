...
- **电商订单导入（CBJ 小程序 + 淘宝）**：上传按表头自动识别平台 → 商品库映射 + Excel 批量导入（预览/确认）+ 关联已有商品或快速新增商品 + 活动标签/赠品（按活动追溯统计）。预览可按全部、待确认、重复、导入、留存、来源更新、跳过筛选；筛选保留已补信息，确认仍处理整批导入/留存/来源更新，更新须逐笔核对。六类标签与退款例外见 [分类边界](docs/order-import-decision-rules.md)。同款促销可追加别名，保留实际成交价；导入预览和订单列表支持**批量补订期**，可筛选、多选、核对后分批补齐缺失日期，并保留审计。商品库采用三段式命名 + 结构化 code（名称与匹配解耦）；**商学院按期发行量**统计包含单期与订阅覆盖期展开、合刊去重。操作步骤见 [用户指南](docs/user-guide.md#2d4-批量补订期)，交付记录见 [进度备忘](docs/order-import-progress.md)。后续重点：财务对账、客户自助下单、其它平台
