...
每周五生成下周一出版的《中国经营报》印数管理表和中通快递发货明细的 Web 应用，并支持管理员上传年度刊期 PDF，预览校验后更新系统刊期表。系统从 V1.1 起还包含**订单管理**模块，V1.2 已支持手工录入、订阅套餐价、active 状态明细编辑与多版本履约方案，下一阶段优先打通订单到 `shipping_details` 的发货同步。
...
- 报数/发货/打包导出时生成的 **审计快照**
- 订单管理中的 **订单 → 明细 → 履约目标** 三级数据（V1.2 支持 active 明细/目标编辑；V1.3 优先做订单 → `shipping_details` 发货同步，其后再做电商批量导入、财务对账、客户自助下单）
