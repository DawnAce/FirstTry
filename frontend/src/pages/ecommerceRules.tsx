...
const SECTIONS: Array<{ h: string; items: ReactNode[] }> = [
  {
    ...
    items: [
      ...
      '待确认：缺少可用下单/支付日期、没有可识别商品行或商品未匹配的新订单，退款和部分退款同样处理。补齐后重新预览；待确认记录不会随本批写入，若先导入其他记录，请保留原文件稍后处理。纯运费和已有来源另按来源规则判断。',
  ...
  },
  {
    h: '投递与运费',
    items: [
      '订阅与运费在同一交易内时，运费计入订单总额，不单建刊物明细；独立纯运费交易只留存，可另行关联订阅，不增加订阅份数。',
      <>同一笔可导入的订阅交易含「中通」运费行时，投递会改为中通并提示核对。<Text strong>独立纯运费留存或关联不会自动转中通</Text>，须到来源交易单独核对投递变更。</>,
    ],
  },
  {
    h: '订单状态导入策略',
    items: [
      ...
      '状态仍无法识别时，当前默认映射为「已付款」；通过其他检查的记录显示「状态未知」警告并归入「可导入」，不会仅因该警告转为待确认，确认前必须核对原始状态。',
    ],
  },
  {
    h: '去重',
