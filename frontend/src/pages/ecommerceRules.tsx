...
const SECTIONS: Array<{ h: string; items: ReactNode[] }> = [
  {
    h: '导入结果标签的边界',
    items: [
      '导入：可确认建单，仍须核对警告；缺订期或单期期号等选填信息也可能属于这一类。',
      '留存：只保存来源交易，不新建业务订单。包括纯运费、已有业务订单补留原件，以及商品或日期未通过校验的退款交易；留存后仍需核对商品、退款金额或关联订阅。',
      '来源更新：同平台、店铺和单号的来源已留存，但本次内容有变化；点开前后信息逐笔核对，确认后保留新版本，不自动修改主订单状态、财务或投递。',
      '重复：同平台、店铺和单号的来源已留存，且内容未变化；不新增来源版本，也不重复建单。只有业务订单、尚无来源原件时，通常归入「留存」。',
      '待确认：未被来源留存规则接收，且缺少可用下单/支付日期、没有可识别商品行或商品未匹配；按该行具体原因处理后重新预览。',
      '跳过：未命中来源留存或更新规则，按待付款、取消/关闭状态或整单忽略商品规则排除；不建单、不留存。纯运费即使待付款或已取消也会留存，已有来源优先比较原件变化。',
      '六类结果互斥，全部 = 导入 + 留存 + 来源更新 + 重复 + 待确认 + 跳过。「跳过」专指规则排除，不是所有未新建订单的合计。',
      '已退款 / 部分退款是商业状态，不直接决定结果。新交易商品和日期通过校验可导入，商品未知或缺日期通常留存；全为忽略商品且日期有效则跳过。已有来源先判断重复或来源更新，以该行具体原因为准。',
    ],
  },
  {
    h: '商品识别与定价',
    items: [
      '未识别商品通常进入「待确认」，退款交易可先留存，已有来源则先比较原件变化。待确认中的同款促销可关联已有商品并追加别名，履约规则不同时再新增商品；保存后重新识别。',
  ...
  },
  {
    ...
    items: [
      ...
      '状态仍无法识别时，当前默认映射为「已付款」；通过其他检查的记录显示「状态未知」警告并归入「导入」，不会仅因该警告转为待确认，确认前必须核对原始状态。',
    ],
  },
  {
    h: '去重',
